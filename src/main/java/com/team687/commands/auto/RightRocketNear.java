...
public class RightRocketNear extends CommandGroup {
  ...
  public RightRocketNear() {
    ...
    // // addParallel( new IntakeOrOuttakeRollers(-SuperstructureConstants.kStowHatchIntakeVoltage, SuperstructureConstants.kStowHatchIntakeVoltage));
    // addSequential(new DriveFalconTrajectory(Robot.drive, AutoConstants.rightRocketNearPathOne, 3, true, 0.27, 0));
    // addSequential(new AutoLiveTargetTrack(0.2, Robot.drive.feetToTicks(2.25, DriveConstants.kLeftTicksPerFoot), 0.00934, 0, 1.75));
    // addSequential(new UltrasonicAutoLiveTargetTrack(0.2, 0.00934, 0, 1.75, 8));
