...
class FirstPython:
    ...
    def __init__(self):
        ...
        self.HSVmin = np.array([56, 193, 45], dtype=np.uint8)
        self.HSVmax = np.array([ 111, 255, 173], dtype=np.uint8)

    
        # Measure your U-shaped object (in meters) and set its size here:
        self.owm = 0.188 # width in meters
        self.ohm = 0.155 # height in meters
        ...
        self.hullfill = 1000                 # Max fill ratio of the convex hull (percent)
    ...
    def detect(self, imgbgr, outimg = None):
        ...
        for c in contours:
            ...
            if len(approx) < 3 or len(approx) > 5: continue  # 8 vertices for a U shape
            str2 += "S" # Shape is ok
