...
public class Drive extends Subsystem {
	...
	public Drive() {
		...
		m_rightMaster.configPIDF(DriveConstants.kRightP, DriveConstants.kRightI, DriveConstants.kRightD,
				DriveConstants.kRightF, 0);
		m_leftMaster.configPIDF(DriveConstants.kLeftP, DriveConstants.kLeftI, DriveConstants.kLeftD, DriveConstants.kLeftF,
				0);
	...
	public void setPowerFeedforward(double leftPower, double rightPower) {
		...
		m_leftMaster.set(ControlMode.PercentOutput, leftPower, DemandType.ArbitraryFeedForward,
				kLeftStatic * Math.signum(leftPower));
		m_rightMaster.set(ControlMode.PercentOutput, rightPower, DemandType.ArbitraryFeedForward,
				kRightStatic * Math.signum(rightPower));
	}
	...
	public void setPositionMotionMagic(double leftPosition, double rightPosition, int acceleration, int cruiseVelocity) {
		...
		m_leftMaster.set(ControlMode.MotionMagic, leftPosition, DemandType.ArbitraryFeedForward,
				DriveConstants.kLeftStatic);
		m_rightMaster.set(ControlMode.MotionMagic, rightPosition, DemandType.ArbitraryFeedForward,
				DriveConstants.kRightStatic);
	}
	...
	public void setVelocity(double leftVel, double rightVel) {
		...
		m_rightMaster.set(ControlMode.Velocity, rightVel, DemandType.ArbitraryFeedForward,
				DriveConstants.kRightStatic * Math.signum(rightVel));
		m_leftMaster.set(ControlMode.Velocity, leftVel, DemandType.ArbitraryFeedForward,
				DriveConstants.kLeftStatic * Math.signum(leftVel));
	}
	...
	public void setVelocityFPS(double leftVel, double rightVel) {
		setVelocity(fpsToTalonVelocityUnits(leftVel, DriveConstants.kTicksPerFootLeft),
				fpsToTalonVelocityUnits(rightVel, DriveConstants.kTicksPerFootRight));
	}
	...
	public void logToCSV() {
		if (!writeException) {
			try {
				...
				// + String.valueOf(getRawYaw()) + "," + String.valueOf(getPitch()) + "," +
				// String.valueOf(getRoll()) +
				// "," + String.valueOf(m_leftSlave1.getMotorOutputVoltage()) + "," +
				// String.valueOf(m_rightSlave1.getMotorOutputVoltage()) +
				// "," + String.valueOf(getLeftVelocityFeet()) + "," +
				// String.valueOf(getRightVelocityFeet()) + "," + String.valueOf(m_currentX) +
				// ","
				// + String.valueOf(m_currentY) + "," + String.valueOf(m_lookaheadX) +"," +
				// String.valueOf(m_lookaheadY) + "," + String.valueOf(getAngularVelocityX()) +
				// "," + String.valueOf(getAngularVelocityY()) +
				// "," + String.valueOf(getAngularVelocityZ()) + "," +
				// String.valueOf(getAccelX()) + "," + String.valueOf(getAccelY()) + "," +
				// String.valueOf(getAccelZ()) + "\n");
				m_writer.flush();
			} catch (IOException e) {
				e.printStackTrace();
				writeException = true;
